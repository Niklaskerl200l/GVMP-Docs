function Besprechung_Start()
{
  ...
  Sheet_Besprechung.getRange("F6").setValue(Utilities.formatDate(new Date(),SpreadsheetApp.getActive().getSpreadsheetTimeZone(),"HH:mm") + " Uhr");
}
...
function Besprechung_Archivieren()
{
  if(SpreadsheetApp.getActive().getSheetName() != "Besprechungsprotokoll Vorlage")
  {
    ...
    var SS_Export = SpreadsheetApp.openById(LSPD.ID_Archiv_Detective_Besprechungen);
...
function Besprechung_Anwesenheit(e)
{
  ...
  if(Spalte == Spalte_in_Index("C") && Zeile >= 6 && Zeile <= 14 && Value == "TRUE" || Spalte == Spalte_in_Index("E") && Zeile >= 6 && Zeile <= 14 && Value == "TRUE")
  {
    Sheet.getRange(Zeile, Spalte).clearDataValidations();
