...
function Personal_Sortieren()
{
  ...
  var Array_Personal = Sheet_Personal.getRange("B4:G18").getValues();
  
  Array_Personal = Array_Personal.sort(Abteilung_Sort);

  for(var y = 0; y < Array_Personal.length; y++)
  {
    Array_Personal[y][0] = '=IF(C' + (y+4) + '="";"";VLOOKUP(C' + (y+4) + ';\'Import Personaltabelle\'!$A$4:$F;2;FALSE))'
    Array_Personal[y][2] = '=IF(C' + (y+4) + '="";"";VLOOKUP(C' + (y+4) + ';\'Import Personaltabelle\'!$A$4:$F;6;FALSE))'
  }

  Sheet_Personal.getRange("B4:G18").setValues(Array_Personal);
  SpreadsheetApp.getActive().getSheetByName("Export Abteilungen").getRange("B11").setValue("WAHR");
}