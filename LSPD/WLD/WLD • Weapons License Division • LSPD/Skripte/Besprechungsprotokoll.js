function Besprechung_Start() 
{
  ...
  var Sheet_Besprechung_Vorlage = SpreadsheetApp.getActive().getSheetByName("Besprechungsprotokoll Vorlage"); //holt den Namen vom Besprechungsprotokoll Vorlage

  var Datum = Utilities.formatDate(new Date(),SpreadsheetApp.getActive().getSpreadsheetTimeZone(),"dd.MM.yyyy"); // holt sich das aktuelle Datum

  var Sheet_Besprechung = Sheet_Besprechung_Vorlage.copyTo(SS) // kopiert es in das aktuelle Blatt

  try
  {
    Sheet_Besprechung.setName("Besprechungsprotokoll " + Datum); //ändert den Namen zu "Besprechungsprotokoll + Datum"
  }
  catch(err)
  {
    try
  ...
  Sheet_Besprechung.getRange("B2").setValue("Besprechungsprotokoll vom " + Datum); //Setzt die Überschrift
  Sheet_Besprechung.getRange("F7").setValue(Utilities.formatDate(new Date(),SpreadsheetApp.getActive().getSpreadsheetTimeZone(),"HH:mm") + " Uhr"); 
}
...
function Besprechung_Archivieren()
{
  if(SpreadsheetApp.getActive().getSheetName() != "Besprechungsprotokoll Vorlage")
  {
    ...
    var SS_Export = SpreadsheetApp.openById("1UtA1JJ2zTL1Kq9461tVFM9JJXzIDgI-BvBknprxoUIs");
...
function Besprechung_Anwesenheit(e)
{
  ...
  if(Spalte == Spalte_in_Index("C") && Zeile >= 6 && Zeile <= 12 && Value == "TRUE" || Spalte == Spalte_in_Index("E") && Zeile >= 6 && Zeile <= 12 && Value == "TRUE")
  {
